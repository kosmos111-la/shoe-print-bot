// В трансформации поворота:
const rotated = SimpleTestData.rotate(eight, 45);

// Эта функция вращает вокруг центра ВСЕЙ фигуры
// Но при этом ОТНОСИТЕЛЬНОЕ положение точек меняется!
```

Пример:
Две точки A и B рядом друг с другом:

· A(100, 100)
· B(110, 100)
· Расстояние: 10px

После поворота вокруг центра (400, 300):

· A становится A'
· B становится B'
· Но расстояние между A' и B' остается 10px!
· Однако их соседи могут измениться!

Проблема: Алгоритм ищет 4 ближайших соседа. После поворота вокруг общего центра, у точки могут появиться другие ближайшие соседы, потому что меняется их расположение относительно друг друга.

Решение: Нужно вращать не вокруг центра фигуры, а сохранять относительные расстояния между точками. Давай исправим код трансформации:
