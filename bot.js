...
// 💾 СИСТЕМА ЭТАЛОНОВ ДЛЯ СРАВНЕНИЯ
...
                totalArea: 0 // Можно добавить расчет площади
            },
            imageUrl: fileUrl,
            timestamp: new Date(),
...
            `📊 Детали: ${predictions.length} элементов\n\n` +
            'Используйте `/list_references` для просмотра'
        );
        return; // Прерываем обычную обработку
       
    } catch (error) {
        await bot.sendMessage(chatId, '❌ Ошибка сохранения эталона');
        session.waitingForReference = null;
        return;
    }
}
