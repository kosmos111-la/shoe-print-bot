...
        );
    } catch (error) {
        console.log('❌ Ошибка в /calc_snow:', error);
        await bot.sendMessage(chatId, '❌ Ошибка загрузки калькулятора');
    }
});

// Команда модуля погоды
...
        await bot.sendMessage(chatId, '❌ Ошибка загрузки модуля погоды');
    }
});
