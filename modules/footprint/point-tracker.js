...
            // 🔥 НОВЫЕ НАСТРОЙКИ СТРОГО ПО ИНСТРУКЦИИ
            directUpdateThreshold: options.directUpdateThreshold || 100,   // 🔥 100 ПО ИНСТРУКЦИИ
            forceUpdateOnMerge: options.forceUpdateOnMerge || true
        };
    }
...
        // Шаг 0: 🔥 ПРЕДВАРИТЕЛЬНОЕ ОБНОВЛЕНИЕ существующих точек СТРОГО ПО ИНСТРУКЦИИ
...
    // 🔥 ИСПРАВЛЕННЫЙ МЕТОД СТРОГО ПО ИНСТРУКЦИИ
    _preUpdateExistingPoints(newPoints, maxDistance = 100) {  // 🔥 100 ПО ИНСТРУКЦИИ
        let updated = 0;
...
                // 🔥 УВЕЛИЧИТЬ ПОРОГ ДО 100 ПО ИНСТРУКЦИИ
...
            // 🔥 УВЕЛИЧИТЬ ПОРОГ ДО 50 ПО ИНСТРУКЦИИ
            if (nearestNewPoint && minDistance < 50) {
