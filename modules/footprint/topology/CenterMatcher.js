...
// 🔥 ПОИСК НАДЁЖНЫХ ТОЧЕК (СМЯГЧЁННЫЕ ПОРОГИ)
...
class CenterMatcher {
    constructor(options = {}) {
        ...
        // 🔥 СМЯГЧЁННЫЕ ПОРОГИ (на основе Precision 66.7%)
        this.reliableMorphThreshold = 0.75;           // было 0.95
        this.reliableLocalThreshold = 0.70;           // было 0.85
        this.minGraphDistanceRatio = 0.5;              // было 0.7 (оставим, но можно 0.5)
        ...
        console.log('🎯 CenterMatcher (смягчённые пороги) создан');
    ...
    findCenterMatches(photoGraph, modelGraph, photoMorphology, modelMorphology) {
        ...
        // 🔥 ЭТАП 2: ФИЛЬТРАЦИЯ (смягчённая)
        const filteredCandidates = candidates.filter(c => {
            // 1. Морфология должна быть достаточно хорошей
            ...
            // 3. Зоны должны совпадать (оставляем жёстко)
