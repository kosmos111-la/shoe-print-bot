...
// 🔥 УНИВЕРСАЛЬНЫЙ СБОРЩИК ПРИЗНАКОВ С ДИАГНОСТИЧЕСКОЙ ТАБЛИЦЕЙ
...
class FeatureExtractor {
    constructor(options = {}) {
        ...
        console.log('   Будет собирать: зона, роль, треугольники, степени, соседи');
    }
    ...
    extractFeatures(node, graph, matchInfo = null) {
        ...
        const features = {
            ...
            neighborRoles: neighbors.map(n => this.getNodeRole(n, 
                this.findNeighbors(n.id, graph), graph)).sort(),
            neighborTriangles: neighbors.map(n => this.countTriangles(
                this.findNeighbors(n.id, graph), graph)).sort((a, b) => b - a),
    ...
    getNodeRole(node, neighbors, graph) {
        ...
        if (degree === 1) return 'L'; // Leaf
        if (degree >= 6) return 'H'; // Hub
        ...
        if (degree >= 3) {
            ...
            if (allConnected) return 'C'; // Clique
        }
    ...
    printFeatureTable(limit = 20) {
        ...
        console.log('┌─────┬─────────────┬─────────────┬─────────┬─────────┬─────────┬─────────┬─────────┬─────────┬─────────┐');
        console.log('│  #  │   КООРД.    │    ЗОНА     │  РОЛЬ   │ ТРЕУГ.  │ СТЕПЕНЬ │   WL    │ РАССТ.  │ ПРАВ.   │');
        console.log('├─────┼─────────────┼─────────────┼─────────┼─────────┼─────────┼─────────┼─────────┼─────────┤');
