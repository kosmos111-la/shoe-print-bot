// modules/footprint/topology/UncertaintyPointManager.js
...
        const weight = Math.min(0.7, newConfidence * 0.7 + 0.2);
...
        );
        // Если смещение большое — увеличиваем радиус, иначе постепенно уменьшаем
        if (displacement > 2) {
            this.radius = Math.min(this.radius * 1.1 + displacement * 0.3, 12.0);
        } else {
            this.radius = Math.max(this.radius * 0.95, 1.0);
        }
    }
   
    /**
