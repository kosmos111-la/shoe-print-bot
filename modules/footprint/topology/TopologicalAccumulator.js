...
    if (existingModel) {
        // 🔥 ЕСЛИ TRANSFORM СИЛЬНО ИЗМЕНИЛСЯ - ПЕРЕСЧИТЫВАЕМ ВСЮ МОДЕЛЬ
        if (existingModel.lastTransform && finalTransform) {
            const scaleChange = Math.abs(finalTransform.scale - existingModel.lastTransform.scale) / existingModel.lastTransform.scale;
            const rotationChange = Math.abs(finalTransform.rotation - existingModel.lastTransform.rotation) * 180 / Math.PI;
           
            // Если масштаб изменился >5% или поворот >10° - пересчитываем модель
            if (scaleChange > 0.05 || rotationChange > 10) {
                console.log(`\n🔄 ОБНАРУЖЕНО СМЕЩЕНИЕ БАЗОВОЙ ОРИЕНТАЦИИ:`);
                console.log(`   • Изменение масштаба: ${(scaleChange*100).toFixed(1)}%`);
                console.log(`   • Изменение поворота: ${rotationChange.toFixed(1)}°`);
                console.log(`   • Пересчитываю все точки и контуры модели...`);
               
                // Вычисляем относительный transform
                const relativeTransform = {
                    scale: finalTransform.scale / existingModel.lastTransform.scale,
                    rotation: finalTransform.rotation - existingModel.lastTransform.rotation,
                    translation: {
                        x: finalTransform.translation.x - existingModel.lastTransform.translation.x,
                        y: finalTransform.translation.y - existingModel.lastTransform.translation.y
                    }
                };
               
                let recalculatedPoints = 0;
                let recalculatedContours = 0;
               
                // Пересчитываем все точки модели
                for (const node of existingModel.graph.nodes.values()) {
                    // Пересчитываем координаты точки
                    const oldX = node.x;
                    const oldY = node.y;
                    const newPos = this.applyTransform({ x: oldX, y: oldY }, relativeTransform);
                    node.x = newPos.x;
                    node.y = newPos.y;
                    recalculatedPoints++;
                   
                    // Пересчитываем контур точки
                    if (node.morphology?.contour && node.morphology.contour.length > 0) {
                        node.morphology.contour = node.morphology.contour.map(p => this.applyTransform(p, relativeTransform));
                        recalculatedContours++;
                    }
                   
                    // Пересчитываем исторические контуры
                    if (node.sourceContours && node.sourceContours.length > 0) {
                        node.sourceContours = node.sourceContours.map(historyItem => {
                            if (historyItem.points && historyItem.points.length > 0) {
                                return {
                                    ...historyItem,
                                    points: historyItem.points.map(p => this.applyTransform(p, relativeTransform)),
                                    retransformed: true
                                };
                            }
                            return historyItem;
...
                }
               
                console.log(`   ✅ Пересчитано ${recalculatedPoints} точек и ${recalculatedContours} контуров модели`);
               
                // Пересчитываем граф после изменения координат
...
                }));
               
                const newGraph = this.graphBuilder.buildGraph(allPoints, 'model_retransformed');
               
...
                }));
               
                console.log(`   ✅ Граф перестроен после смены ориентации`);
            }
        }
       
