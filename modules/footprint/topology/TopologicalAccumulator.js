...
    // 🔥🔥🔥 УЛУЧШЕНИЕ МОДЕЛИ С ТРИАНГУЛЯЦИОННЫМ ВОССТАНОВЛЕНИЕМ
...
        // Добавляем узлы в модель с восстановленными координатами
...
            // Если узел уже в маппинге - пропускаем
...
            // Если узел связан с 2+ совпавшими узлами - добавляем
...
    // 🔥 ПОДСЧЕТ СВЯЗЕЙ С СОВПАВШИМИ УЗЛАМИ
...
            console.log(`   ${tri} ${nodeId} ${source}: (${node.x?.toFixed(1) || '?'}, ${node.y?.toFixed(1) || '?'}) | степень: ${node.degree} | подтверждений: ${node.confirmationCount || 1}`);
        }
