...
// 🔥 ТОПОЛОГИЧЕСКАЯ ВИЗУАЛИЗАЦИЯ (с диагностикой)
...
            console.log('🔍 Диагностика данных визуализации:');
            console.log(`   Всего точек: ${topologyData.points.length}`);
            console.log(`   Статистика:`, topologyData.stats);
          
            // Проверить первые 5 узлов
            const pointsWithData = topologyData.points.filter(p => p && p.vizData);
            console.log(`   Узлов с vizData: ${pointsWithData.length}/${topologyData.points.length}`);
          
            if (pointsWithData.length > 0) {
                pointsWithData.slice(0, 3).forEach((point, idx) => {
                    console.log(`   Узел ${idx}: ${point.id || 'без ID'}`);
                    console.log(`     координаты: (${point.x?.toFixed(1) || '?'}, ${point.y?.toFixed(1) || '?'})`);
                    console.log(`     подтверждений: ${point.vizData?.confirmations || 0}`);
                    console.log(`     уровень: ${point.vizData?.confirmationLevel || 'unknown'}`);
                });
            }
          
...
                    console.log(`   - Узлов отрисовано: ${topologyData.points.length}`);
                    console.log(`   - Статистика подтверждений сохранена`);
                  
                    resolve({
                        path: outputPath,
...
        const points = topologyData.points.filter(p => p && p.x && p.y);
        if (points.length === 0) {
            console.log('⚠️ Нет точек с координатами для визуализации');
            return;
        }
      
...
        const centerY = canvasHeight * 0.6;
      
        console.log(`🔍 Отрисовка ${points.length} узлов:`);
...
                ctx.font = '8px Arial';
                ctx.textAlign = 'center';
                ctx.fillText(point.id?.substring(0, 6) || 'node', x, y + size + 10);
            }
        });
      
...
            console.log('⚠️ Нет валидных точек для расчета границ');
...
                const pointsWithCoords = topologyData.points.filter(p => p && p.x && p.y);
                report += `📍 ИНФОРМАЦИЯ О УЗЛАХ:\n`;
                report += `• Всего узлов в данных: ${topologyData.points.length}\n`;
                report += `• Узлов с координатами: ${pointsWithCoords.length}\n`;
                report += `• Узлов с vizData: ${topologyData.points.filter(p => p && p.vizData).length}\n\n`;
              
                // Примеры узлов
                report += `📋 ПРИМЕРЫ УЗЛОВ (первые 5):\n`;
                topologyData.points.slice(0, 5).forEach((point, idx) => {
                    report += `• Узел ${idx + 1}: ${point.id || 'без ID'}\n`;
                    report += `  Координаты: (${point.x?.toFixed(1) || '?'}, ${point.y?.toFixed(1) || '?'})\n`;
                    report += `  Подтверждений: ${point.vizData?.confirmations || 0}\n`;
                    report += `  Уровень: ${point.vizData?.confirmationLevel || 'unknown'}\n`;
                });
                report += `\n`;
            }
          
