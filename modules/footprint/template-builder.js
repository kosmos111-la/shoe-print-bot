// template-builder.js - ИСПРАВЛЕННАЯ ВЕРСИЯ
...
        this.config = {
            minPointsForReference: options.minPointsForReference || 5, // БЫЛО: 15
            cellSize: options.cellSize || 15, // БЫЛО: 20
            confirmationThreshold: options.confirmationThreshold || 2,
            highConfidenceThreshold: options.highConfidenceThreshold || 3,
            maxAlignmentError: options.maxAlignmentError || 50, // БЫЛО: 0.15 (слишком строго!)
...
        if (!transformation) {
            console.log(`❌ Не удалось найти трансформацию`);
            return false;
        }

        // 🔥 ВРЕМЕННО ИГНОРИРУЕМ ОШИБКУ для тестирования
        console.log(`📊 Ошибка трансформации: ${transformation.error?.toFixed(1) || 'unknown'} ` +
                   `(порог: ${this.config.maxAlignmentError})`);

        // ПРОДОЛЖАЕМ даже с ошибкой!
        // if (transformation.error > this.config.maxAlignmentError) {
        //     console.log(`❌ Ошибка совмещения слишком велика: ${transformation.error}`);
        //     return false;
        // }
...
    // 🔥 МЕТОД: ПОСТРОИТЬ АДАПТИВНУЮ СЕТКУ (ИСПРАВЛЕННЫЙ!)
    buildAdaptiveGrid() {
        console.log(`🔲 Строю адаптивную сетку из ${this.referencePoints.length} точек...`);

        // 1. DBSCAN КЛАСТЕРИЗАЦИЯ для определения плотности
        const clusters = this.performDBSCAN(this.referencePoints, 10, 1); // eps=10, minPts=1

        if (clusters.length === 0) {
            console.log(`⚠️ DBSCAN не нашел кластеров! Создаю простую сетку...`);
            return this.createSimpleGrid(); // 🔥 ФАЛЛБЭК!
        }

        // 2. СОЗДАТЬ ЯЧЕЙКИ ВОКРУГ КЛАСТЕРОВ
        clusters.forEach((cluster, clusterIndex) => {
            if (cluster.points.length < 1) return; // БЫЛО: 2
...
    // 🔥 НОВЫЙ МЕТОД: ПРОСТАЯ СЕТКА (фаллбэк)
    createSimpleGrid() {
        console.log(`🔄 Создаю простую сетку на основе всех точек...`);

        // Просто создаем ячейку для каждой точки
...
        console.log(`✅ Создано ${this.referencePoints.length} ячеек (простая сетка)`);
    }
...
    // 🔥 МЕТОД: НАЙТИ ТРАНСФОРМАЦИЮ К ЭТАЛОНУ (ИСПРАВЛЕННЫЙ!)
    findTransformation(points, graphId) {
        console.log(`🔍 Ищу трансформацию для графа ${graphId} (${points.length} точек)`);

        if (!this.referencePoints || this.referencePoints.length === 0) {
            console.log(`⚠️ Нет точек эталона для трансформации`);
            return null;
        }

        if (points.length < 2) {
            console.log(`⚠️ Слишком мало точек для трансформации: ${points.length}`);
            return null;
        }

        // 🔥 ВРЕМЕННОЕ РЕШЕНИЕ: только смещение по центрам
        const centerPoints = this.calculateCenter(points);
        const centerReference = this.calculateCenter(this.referencePoints);

        const translation = {
            x: centerReference.x - centerPoints.x,
            y: centerReference.y - centerPoints.y
        };

        // Простая оценка ошибки
        let error = 0;
        if (points.length > 0 && this.referencePoints.length > 0) {
            // Среднее расстояние после трансформации
            const distances = [];
            points.forEach(point => {
                const transformedX = point.x + translation.x;
                const transformedY = point.y + translation.y;

                // Ищем ближайшую точку эталона
                let minDist = Infinity;
                this.referencePoints.forEach(refPoint => {
                    const dx = transformedX - refPoint.x;
                    const dy = transformedY - refPoint.y;
...
            error = distances.reduce((sum, dist) => sum + dist, 0) / distances.length;
        }

        console.log(`📐 Упрощенная трансформация: смещение=(${translation.x.toFixed(1)}, ${translation.y.toFixed(1)}), ` +
                   `ошибка=${error.toFixed(1)}`);
...
        // Если нет ячеек, создаем временные
        if (this.templateCells.size === 0) {
            console.log(`⚠️ Нет ячеек шаблона! Создаю временные ячейки...`);
            this.createTemporaryCellsFromPoints(alignedPoints, graphId);
        }
...
                const searchRadius = cell.radius || this.config.cellSize * 2; // В 2 раза больше
...
                });
            } else {
                console.log(`⚠️ Точка ${point.id} не сопоставлена ни с одной ячейкой`);
            }
        });
...
    // 🔥 НОВЫЙ МЕТОД: временные ячейки
    createTemporaryCellsFromPoints(points, graphId) {
        points.forEach((point, index) => {
            const cellId = `temp_cell_${graphId}_${index}`;
            this.templateCells.set(cellId, {
                center: { x: point.x, y: point.y },
                radius: this.config.cellSize / 2,
                points: [point.id],
...
        console.log(`🆕 Создано ${points.length} временных ячеек для графа ${graphId}`);
    }
...
    // 🔥 МЕТОД: ВСПОМОГАТЕЛЬНЫЙ - DBSCAN (ИСПРАВЛЕННЫЙ!)
    performDBSCAN(points, eps = 15, minPts = 1) { // БЫЛО: eps = this.config.cellSize * 1.5 (45), minPts = 2
        console.log(`🔍 DBSCAN: ${points.length} точек, eps=${eps}, minPts=${minPts}`);
...
        console.log(`✅ DBSCAN найдено ${clusters.length} кластеров, шум: ${noise.size} точек`);
        return clusters;
    }
