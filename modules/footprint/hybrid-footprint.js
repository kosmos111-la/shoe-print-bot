...
        console.log(`   🧭 Векторы: ${this.getVectorCount()} точек`);
        console.log(`   🎯 Трекер: ${this.pointTracker.getStats().totalPoints} точек с рейтингами`);
...
        const matrixFeatures = this.distanceMatrix.extractFeatures();
        this.stats.matrixConfidence = matrixFeatures ?
            (matrixFeatures.symmetry * 0.5 + matrixFeatures.density * 0.5) : 0.5;

        // Уверенность на основе векторной схемы (используем безопасный доступ)
        const vectorStats = this.getVectorStats();
        this.stats.vectorConfidence = vectorStats.coverage || 0.5;
...
            this.stats.vectorConfidence * 0.25 +
            this.stats.graphConfidence * 0.3
        );
...
        if (vectorResult.similarity < 0.5) {
            console.log(`🚫 Отсев по векторной схеме (similarity: ${vectorResult.similarity.toFixed(3)})`);
            return {
...
        // ШАГ 5: ГРАФ - только если всё остальное похоже
...
            bitmaskResult.similarity * 0.1 +
            momentResult.similarity * 0.15 +
            matrixResult.similarity * 0.2 +
            vectorResult.similarity * 0.25 +
            graphResult.similarity * 0.3
        );
...
    // 5. БЫСТРЫЙ ОТСЕВ
    quickReject(step, result, steps, startTime) {
        return {
            similarity: result.similarity || 0,
            decision: 'different',
            reason: `Быстрый отсев на шаге ${step}`,
            steps,
            fastReject: true,
            timeMs: Date.now() - startTime
        };
...
    // 6. ОБЪЕДИНЕНИЕ С ТРАНСФОРМАЦИЕЙ
    mergeWithTransformation(otherFootprint) {
        console.log(`🔄 Объединяю с трансформацией "${otherFootprint.name}"...`);
...
        // 4. Получить точки высокого доверия из трекера
        const highConfidencePoints = this.pointTracker.getHighConfidencePoints(0.5);

        // 5. Пересчитать все представления из точек высокого доверия
        const pointsForRecalculation = highConfidencePoints.map(pt => ({
            x: pt.x,
            y: pt.y,
            confidence: pt.rating || 0.5
        }));

        // Обновить все представления
        this.bitmask.createFromPoints(pointsForRecalculation);
        this.moments.calculateFromPoints(pointsForRecalculation);
        this.distanceMatrix.createFromPoints(pointsForRecalculation);
        this.vectorGraph.createFromPoints(pointsForRecalculation);
        this.graph.buildFromPoints(pointsForRecalculation);

        // Обновить оригинальные точки
        this.originalPoints = pointsForRecalculation;
...
        console.log(`✅ Успешно объединено с трансформацией!`);
        console.log(`   📊 Добавлено точек: ${trackerResult.added}`);
        console.log(`   🔄 Обновлено точек: ${trackerResult.updated}`);
        console.log(`   🎯 Точек высокого доверия: ${highConfidencePoints.length}`);
        console.log(`   💎 Общая уверенность: ${Math.round(this.stats.confidence * 100)}%`);
...
    // 7. ПРОСТОЕ ОБЪЕДИНЕНИЕ (без трансформации)
    mergeSimple(otherFootprint) {
        console.log(`🔄 Простое объединение с "${otherFootprint.name}"...`);

        // Объединить точки
        const combinedPoints = [...this.originalPoints, ...otherFootprint.originalPoints];

        // Пересчитать всё
        this.createFromPoints(combinedPoints, { source: 'merge_simple' });
...
        return {
            success: true,
            mergedPoints: combinedPoints.length,
            originalPoints: this.originalPoints.length,
            addedPoints: otherFootprint.originalPoints.length
        };
    }
...
                vectorPoints: this.getVectorCount(),
                trackedPoints: this.pointTracker.getStats().totalPoints
            }
        };
    }
...
                console.log(`${i+1}. ${step.step}: ${step.result?.similarity?.toFixed(3) || 'unknown'} (${step.time}ms)`);
            });
        }
...
    // 13. БЫСТРЫЙ ПОИСК ПО БИТОВОЙ МАСКЕ (для базы данных)
...
    getVectorCount() {
        try {
            // Попробовать метод getVectorCount
            if (typeof this.vectorGraph.getVectorCount === 'function') {
                return this.vectorGraph.getVectorCount();
            }
            // Если метод отсутствует, попробовать получить данные из starVectors
            if (this.vectorGraph.starVectors && Array.isArray(this.vectorGraph.starVectors)) {
...
    // Получить статистику векторов (безопасно)
    getVectorStats() {
        try {
            if (typeof this.vectorGraph.getStats === 'function') {
                return this.vectorGraph.getStats();
            }
            // Возвращаем значения по умолчанию
            return {
                totalPoints: this.vectorGraph.points ? this.vectorGraph.points.length : 0,
                starVectors: this.vectorGraph.starVectors ? this.vectorGraph.starVectors.length : 0,
                coverage: 0.5,
                avgVectorsPerPoint: 0
            };
        } catch (error) {
            console.log('⚠️ Ошибка при получении статистики векторов:', error.message);
            return { coverage: 0.5 };
        }
    }
...
    getMatrixSizeString() {
        try {
            if (typeof this.distanceMatrix.getSizeString === 'function') {
                return this.distanceMatrix.getSizeString();
            }
            if (this.distanceMatrix.matrix && Array.isArray(this.distanceMatrix.matrix)) {
