...
    // 🔥 АКТИВНАЯ СИНХРОНИЗАЦИЯ ВСЕХ МОДУЛЕЙ
    synchronizeAllModules() {
        console.log('\n🎬 АКТИВНАЯ СИНХРОНИЗАЦИЯ ВСЕХ МОДУЛЕЙ:');

        if (!this.manager) {
            console.log('⚠️ Нет менеджера для синхронизации');
            return;
        }

        let synchronized = 0;

        // 1. Синхронизируем vectorSuperModels
        if (this.manager.vectorSuperModels) {
            this.manager.vectorSuperModels.forEach((vectorModel, userId) => {
                console.log(`🔄 Синхронизирую VectorModel для ${userId}...`);

                // Исправляем templateBuilder
                if (vectorModel.templateBuilder) {
                    const builder = vectorModel.templateBuilder;

                    // Исправляем normalizationTransform
                    if (builder.normalizationTransform) {
                        const corrected = this.enforceCanonicalSystem(
                            `vector_model_${userId}_norm`,
                            builder.normalizationTransform
                        );
                        builder.normalizationTransform = corrected;
                        synchronized++;
                    }

                    // Исправляем референсный граф
                    if (builder.referenceGraph?.transformation) {
                        const corrected = this.enforceCanonicalSystem(
                            `vector_model_${userId}_ref`,
                            builder.referenceGraph.transformation
                        );
                        builder.referenceGraph.transformation = corrected;
                        synchronized++;
                    }
                }
            });
...
        // 2. Синхронизируем загруженные модели
        if (this.manager.loadedModels) {
            this.manager.loadedModels.forEach((footprint, id) => {
                if (footprint.transformation && !this.isCanonical(footprint.transformation)) {
                    const corrected = this.enforceCanonicalSystem(
...
    // 🔥 ПРИНУДИТЕЛЬНАЯ СИНХРОНИЗАЦИЯ ПЕРЕД СРАВНЕНИЕМ
    forceSynchronizeBeforeComparison() {
        console.log('\n🎬 ПРИНУДИТЕЛЬНАЯ СИНХРОНИЗАЦИЯ ПЕРЕД СРАВНЕНИЕМ:');

        // 1. Сначала синхронизируем все модули
        this.synchronizeAllModules();

        // 2. Затем проверяем что ВСЕ системы канонические
        const audit = this.auditAllSystems();

        // 3. Если все еще есть проблемы - форсированная коррекция
        if (!audit.allCanonical) {
            console.log('🚨 ОБНАРУЖЕНЫ РАСХОЖДЕНИЯ! Форсированная коррекция...');
            this.applyGlobalCorrections();
        }

        console.log('✅ Системы гарантированно синхронизированы для сравнения');
        return true;
    }
