...
    x: 600 - (p.x - 300), // 🔴 ПРАВИЛЬНОЕ ЗЕРКАЛО: отражение относительно x=300
    y: p.y + 100,
    confidence: p.confidence,
    id: `${p.id}_mirrored`
}));
...
// 🔥 УТОЧНЯЕМ ОЖИДАНИЯ:
const expectedDx = 300; // Так как зеркалим относительно x=300, x' = 600-x
const expectedDy = 100;

printResult(result3, 0, expectedDx, expectedDy);

// 🔥 ОСОБАЯ ПРОВЕРКА ЗЕРКАЛА:
console.log('\n🔍 ДИАГНОСТИКА ЗЕРКАЛА:');
if (result3.mirrored) {
    console.log('✅ ЗЕРКАЛО ОБНАРУЖЕНО!');
    if (result3.score >= 0.7) {
        console.log(`🎯 Отличный score: ${(result3.score * 100).toFixed(1)}%`);
    } else {
        console.log(`⚠️ Score низкий, но зеркало обнаружено: ${(result3.score * 100).toFixed(1)}%`);
    }
} else {
    console.log('❌ ЗЕРКАЛО НЕ ОБНАРУЖЕНО');
    console.log(`📊 Score: ${(result3.score * 100).toFixed(1)}%`);
   
    // Проверяем, не является ли это ложным отрицанием
    if (result3.score > 0.6) {
        console.log('⚠️ Высокий score без обнаружения зеркала - возможно ошибка алгоритма');
    }
}
